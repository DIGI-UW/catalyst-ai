...
describe("Catalyst query workflow", () => {
  ...
  it.each([
    ...
    },
    {
      mode: "legacy preview",
      title: "Preparing your question",
      message: "We’re checking your question and proposed query. No data has been retrieved.",
    },
  ] as const)(
  ...
  it("restores the notebook after refresh and New Session removes every prior turn context", async () => {
    ...
    // The restored turn remains visible and anchored; query evidence opens separately.
    expect(
      screen.getByRole("region", { name: "Query turn 1" }),
    ).toBeVisible();
  ...
  it("uses an available revision profile after restoring a legacy session without rewriting its turn snapshot", async () => {
    ...
    render(<App api={api} />);
    await enableAdvancedMode();
    await user.click(await screen.findByText("View query details"));
