...
const technical = () => `<details><summary>Query and technical details</summary><p class="small">Source: ${escapeHtml(source)}<br>SQL dialect: Spark SQL<br>Query version: 1 · execution: 1</p><pre>${escapeHtml(draftSql)}</pre><p class="small">Schema and trace identifiers are available here in the product. This mock has no live provenance.</p></details>`;
const sqlEditor = () => `<details id="sql-details"><summary>View or edit SQL</summary><label for="sql">SQL query</label><textarea class="sql" id="sql" spellcheck="false">${escapeHtml(draftSql)}</textarea><p class="small muted">Sample SQL only. The mock does not validate or execute it.</p><div class="actions"><button class="secondary" data-action="validate">Validate</button><button class="secondary" data-action="format">Format</button><button class="text-button" data-action="restore">Restore</button><button class="text-button" data-action="clear">Clear draft</button></div><details><summary>Parameters and generation details</summary><p>No parameters in this example. The product preserves typed parameters, model output, findings, and exact query history.</p></details></details>`;
...
function render() {
  ...
  if (state === 'empty') {
    ...
    if (state === 'ready') {
      ...
      body += `<div class="card"><div class="card-header"><div><h2>Your results are ready</h2><p class="muted">${escapeHtml(question)}</p></div><span class="tag success">Query ran</span></div>${facts()}${state === 'limited' ? '<div class="callout warning"><strong>Showing the first 100 rows</strong><p>More are available; the total is unknown. Refine your question if you need a smaller result.</p></div>' : ''}<p class="small">No SQL findings recorded. AI review status and generation evidence remain available in query details.</p><div class="actions"><button data-dialog="review">Review results and save</button><button class="secondary" data-action="edit">View or edit SQL</button></div><details><summary>Query details</summary><p class="small">A successful run means the database returned a result. It does not establish that the answer is correct.</p>${technical()}</details></div>`;
    }
  }
...
function openPanel(kind, trigger) {
  ...
  footer.innerHTML = '';
  const titles = {review:'Review results',ai:'AI setup and details',data:'Available data',source:'Start a new session',widget:'Create Widget',import:'Import into Superset'};
  $('#panel-title').textContent = titles[kind];
  if (kind === 'review') {
    $('#panel-body').innerHTML = `<label for="dataset-name">Dataset name</label><input id="dataset-name" value="${escapeHtml(datasetName)}">${facts()}${state === 'limited' ? '<div class="callout warning">100 rows returned. More are available; the total is unknown. The mock illustrates six rows below.</div>' : ''}${table()}<p class="small muted">Saving keeps this query and its recorded execution. Superset runs the saved query against the connected source when the dashboard is viewed.</p><p class="small">Database diagnostic: none in this example.<br>SQL findings: none recorded in this example.</p>${technical()}`;
    footer.innerHTML = `<button data-action="save" ${saved ? 'disabled' : ''}>${saved ? 'Saved to Datasets' : 'Save Dataset'}</button><button class="secondary" data-action="close">Close</button>`;
  } else if (kind === 'ai') {
    ...
    footer.innerHTML = '<button data-action="save-widget">Save Widget</button>';
  } else {
...
document.addEventListener('change', event => {
  if (event.target.id === 'profile') {
    ...
    $('[data-dialog="ai"]').textContent = `AI: ${profile}`;
  }
});
