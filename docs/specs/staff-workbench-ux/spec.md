...
| Submit | “Prepare query” creates a candidate without running it. Ctrl/Command + Enter invokes the same action; plain Enter inserts a newline. Empty input cannot submit. |
...
Keep the names **Workbench**, **Datasets**, **Widgets**, and **Dashboards**.
Explain Datasets as “Saved queries” and Widgets as “Charts and tables”. This
proposal does not start a navigation-renaming project.

“Browse available data” opens the existing searchable, filterable, paginated
...
Keep a compact visible label for the selected AI setup and an explicit details
control. The opened panel exposes the available profile chooser, exact models,
and recorded generation information. Profile selection stays explicit; there is
no automatic team choice, substitution, or fallback. Distinguish AI-prepared
content from manual edits.
...
| --- | --- |
| Prepared | “Query ready for review”, one “View or edit SQL” disclosure, and explicit “Run query”. Retain the single editor, formatting, wrapping, typed parameters, Validate, Clear/Restore, and history. No result exists yet. |
...
| Success | “Query ran”, source, returned-row summary, and a review action. Execution success is not proof that the answer is correct or clinically approved. |
...
5. Save Dataset / Close footer with an explicit saved state.
...
Empty libraries offer the next possible action. Widgets directs users to a
saved Dataset, or back to their question if none exists. Dashboards directs
users to create/add Widgets. Disabled buttons do not explain prerequisites.
...
The offline mock uses semantic native controls and Carbon-aligned tokens without
