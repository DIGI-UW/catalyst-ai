...
is discarded: warmup opens no user stores, creates no sessions or previews, and
...
- The current editor and earlier successful result remain available after a
  generation failure.
- A detected client disconnect cancels work and records `generation_cancelled`;
  no client response can be relied on after disconnect. It releases the claimed
  turn and retains available completed/cancelled role evidence. It does not
  start repairs.
