...
Question preparation remains active until it completes or the person stops it.
A lost request cancels outstanding model work and records the claimed turn with
the cancellation reason without changing the selected query or prior result. The
session becomes available for another turn. **Stop preparing** in the initial or
follow-up composer cancels its request, keeps the text and selected query, and
returns focus to the input with a Retry action. Unmounting the Workbench also
...
Published tables set their row limit one above the complete immutable file
count, so Superset does not mistake a complete file for a truncated result.
